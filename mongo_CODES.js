...
db.companies.find({ category_code: "music" }, {name:1}).sort({ name: 1, _id: 1 });
...
db.sales.find({ $and: [{storeLocation: 'London'}], $or: [{'items.name': 'laptop', 'items.name':'backpack', 'items.name': 'printer paper'}]} ).sort({saleDate: 1}).limit(3)
...
db.sales.find(
  {$and: [{"customer.age": {$lt: 30}}, {"customer.satisfaction": {$gt: 3}}]},
